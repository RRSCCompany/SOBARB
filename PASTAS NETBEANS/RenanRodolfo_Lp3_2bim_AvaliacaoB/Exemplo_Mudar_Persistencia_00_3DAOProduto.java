...
public class DAOProduto extends DAOGenerico<Produto> { 
    ...
    public List<Produto> listByNome(String nome) {
        return em.createQuery("SELECT e FROM Produto e WHERE e.nome) LIKE : nomeProduto").setParameter("nome", "%" +nome+ "%").getResultList();
    }
    public List<Produto> listById(int id) {
        return em.createQuery("SELECT e FROM Produto e WHERE e.id = : id " ).setParameter("id", id).getResultList(); 
    }
