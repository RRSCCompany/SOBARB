...
public class DAOPessoa extends DAOGenerico<Pessoa> { 
    ...
    public List<Pessoa> listByNome(String nome_pessoa) {
        ...
        return em.createQuery("SELECT e FROM Pessoa e WHERE e.nome_pessoa) LIKE : nomePessoa").setParameter("nome", "%" +nome_pessoa+ "%").getResultList();
    }

    public List<Pessoa> listById(int id) {
        return em.createQuery("SELECT e FROM Pessoa e WHERE e.id_pessoa = : id " ).setParameter("id", id).getResultList(); 
    }
